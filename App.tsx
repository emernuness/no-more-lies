...
function App() {
  const { isListening, startListening, stopListening, transcript, finalTranscript, error: speechError } = useSpeechRecognition();
  ...
  useEffect(() => {
    if (finalTranscript && !processedTexts.has(finalTranscript) && finalTranscript.trim().length > 10) {
      handleVerification(finalTranscript);
      setProcessedTexts(prev => new Set(prev).add(finalTranscript));
    }
  ...
  const handleVerification = async (text: string) => {
    ...
    try {
      const resultData = await checkFact(text);
            ...
            <div className="bg-zinc-900 rounded-2xl p-6 border border-zinc-800 shadow-xl flex flex-col items-center">
                ...
                {speechError ? (
                    <div className="w-full mb-4 p-4 bg-red-500/10 border border-red-900/50 rounded-lg flex flex-col items-center gap-2 text-center animate-in fade-in slide-in-from-top-2">
                        ...
                        <span className="text-sm font-semibold text-red-400">Erro de Conexão</span>
